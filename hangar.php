...
$result=mysql_query($c)or die(mysql_error());
echo '<table width="100%"><tr><td><b>Nombre</b></td><td><b>Posición</b></td>';
while ($p = mysql_fetch_array($result)){echo "<tr><td><a href=\"proyecto.php?id=vehiculo&div=$p[nombre]\">$p[nombre]</a></td><td>($p[x],$p[y]) $p[ciudad] <a onMouseover=\" ddrivetip('Poner a la venta', '#808080');\" onMouseout=\"hideddrivetip()\"  href=\"clanact.php?act=vender&vehiculo=$p[nombre]\"><img border=0 src=\"images/arr.gif\"><small>Vender</small></a></td></tr>";} 
echo '</table>';
...
while ($p = mysql_fetch_array($result)){echo "<tr><td><a href=\"proyecto.php?id=vehiculo&div=$p[nombre]\">$p[nombre]</a></td><td><small>$p[tprop] $p[prop]</small></td></tr>";} 
echo '</table>';
...
while ($p = mysql_fetch_array($result)){echo "<tr><td><a href=\"proyecto.php?id=vehiculo&div=$p[nombre]\">$p[nombre]</a></td><td><small>$p[tprop] $p[prop]</small></td><td>$p[precio] <a onMouseover=\" ddrivetip('Comprar', '#808080');\" onMouseout=\"hideddrivetip()\" href=\"comprar.php?veh=$p[nombre]\"><img border=0 src=\"images/arr.gif\"></a></td></tr>";} 
echo '</table>';
