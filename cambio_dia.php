...
if ($_GET[pass]=="muchcode") {
    ...
    $log.='<br>Poner el maximo de cr&eacute;ditos recibidos a 0';
    ...
    while ($cy=mysql_fetch_array($result3)) {
        if ($cy[clan] != '') {
            ...
            if ($cn[fondos]<0) {
                ...
                $sql = "INSERT INTO `sw_mess` (emisor, receptor, fecha, mess) VALUES ('INFORMACION', '$cn[lider]', '$fe[val]', 'EL CLAN HA SIDO SUSPENDIDO POR INSOLVENCIA')";
                $result = mysql_query($sql);

                $sql = "INSERT INTO `sw_log` (user, log, dia, tipo, ref) VALUES ('$cn[Lider]', 'Tu clan ha sido eliminado por Insolvencia.', '$fe[val]', '1', '')";
                $result = mysql_query($sql);
...
#                                       recalcular potencias->
    ...
    $log.= "<br>Gesti&oacute;n clanes...ok";
...
#   fin
    mysql_query("INSERT INTO sw_control (fecha, log) VALUES ('Dia $fe[val]', '$log')")or die(mysql_error());
