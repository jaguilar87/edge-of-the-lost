...
$result=mysql_query($c)or die(mysql_error());
$sql = "INSERT INTO `sw_clanboard` (clan, poster, dia, mess) VALUES ('$us[clan]', 'INFORMACION', '$us[dia]', '$us[nombre] ha donado $_GET[cre] Créditos al Clan!')";
$result = mysql_query($sql);
}}
...
if ($cl[lider]==$us[nombre]){ 
$c="SELECT * FROM `sw_clanboard` WHERE id='$_GET[mess]'";
$result=mysql_query($c)or die(mysql_error());
$ider=mysql_fetch_array($result);
if ($ider[clan]==$cl[nombre]){
   $sql = "DELETE FROM sw_clanboard WHERE id='$_GET[mess]'";
   $result = mysql_query($sql);
   echo "Mensaje Borrado";
echo "<script> location.href=\"gestionar.php#mess\" </script>";
...
echo "<script> location.href=\"ciudad.php?ciudad=$_GET[ci]\" </script>";
}else{echo "No eres el lider del Clan.";}
...
if ($cl[lider]==$us[nombre]){ 
if ($_GET[pago]<0 ||$_GET[pago] > 1000){echo 'Valor NO valido o superior a 1000C';}else{
...
if ($_GET[vehiculo]==""){echo "<script> location.href=\"hangar.php\" </script>";}
...
if($cl[lider]==$us[nombre]){
		...
		if($uc[nombre]==$_GET[vehiculo] || $ul[nombre]==$_GET[vehiculo]){$v=true;}
...
echo "<br>Volver a la pantalla de <a href=\"clan.php?clan=$us[clan]\">Clan</a>";
include 'footer.php'; ?>