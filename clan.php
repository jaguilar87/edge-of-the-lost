...
if ($cl[lider]==$us[nombre] && $us[clan]==$_GET[clan]){ echo '<center>[<a href="gestionar.php">Gestionar Clan</a>]</center>';}
...
$result=mysql_query($c)or die(mysql_error());
$i=1;
echo '<table width="100%"><tr><td><b>Nombre</b></td><td><b>Planeta (x,y)</b></td><td><b>Dirigente</b></td><td><small>Mina</small></td></tr>';
while ($r = mysql_fetch_array($result)){$u=textcolor($r[rey]); $d="SELECT * FROM `sw_plan` WHERE posx='$r[esx]' AND posy='$r[esy]'"; $res=mysql_query($d)or die(mysql_error()); $x = mysql_fetch_array($res); echo "<tr><td>$i- <a href=\"ciudad.php?ciudad=$r[nombre]\">$r[nombre]</a></td><td>$x[nombre] ($r[esx],$r[esy])</td><td><a href=\"info.php?us=$r[rey]\"><font color=\"$u[txtc]\">$r[rey]</font></a></td><td>$r[mina]</td></tr>"; $i++;}
echo '</table>';
