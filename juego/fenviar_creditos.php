	 ...
	 if ($_GET[cr]<=0){
	 		...
	 		if ($us[creditos]<$_GET[cr]){
				 ...
				 if ($usto[crecived]>10000){
				 		echo "$usto[nombre] ya ha recibido el limite de créditos por hoy";
				 }else{ 
				 	  mysql_query("UPDATE `sw_users` SET creditos='$usto[creditos]', crecived='$usto[crecived]' WHERE nombre='$_GET[to]'")or die(mysql_error());
				 		...
				 		mysql_query("INSERT INTO `sw_log` (user, log, dia, ref, tipo) VALUES ('$usto[nombre]', '$us[nombre] te ha enviado $_GET[cr] Créditos.', '$us[dia]', '$us[nombre]', '3')")or die(mysql_error());
