...
if ($ok){
   if ($_GET[ci]!="" && $_GET[pl]!=""){
	  ...
	  if($us[creditos]<0 || $us[turnos]<0){echo 'Creditos y/o energ�a insuficientes...';}else{
	  ...
	  if ($_GET[clan] && $vev[tprop]=="Jugador" && $us[nombre]==$vev[prop] && $us[nombre]==$cl[lider]){
	     mysql_query("UPDATE sw_users SET ciudad='$_GET[ci]', planeta='$plc[nombre]' WHERE clan='$us[clan]' AND ciudad='$us[ciudad]'")or die(mysql_error());
	        ...
	        mysql_query("UPDATE sw_users SET turnos='$us[turnos]', creditos='$us[creditos]' WHERE nombre='$us[nombre]'")or die(mysql_error()); 
	  		echo "Viaje a $_GET[ci] completado, ha pagado $coste Cr�ditos.<br><br><center> $un[mess] </center>";
	  }
   }
}else{ 
	  ...
	  if ($_GET[ci]!="" && $_GET[pl]!=""){
		 ...
		 echo'<br><br><i>Veh�culo Pr�pio:</i>';
		 ...
		 if ($us[nombre]==$cl[lider]){
		 	echo '<br><br><input type="checkbox" name="clan" value="Check me">Traerse a todo el Clan (S�lo con veh�culo propio)';
		 }
