...
if ($ci[mina]=="S" && $us[nombre]==$ci[rey]){
 ...
 if ($_GET[pot]){
	...
	$pl[mineral]-=$_GET[pot];
 	...
 	if ($cl[potencia]<0){
	   echo "El clan no dispone de suficiente Potencia para realizar esa acci�n";
	}else{
	   if ($pl[mineral]<0){
			 ...
			 mysql_query("UPDATE sw_plan SET mineral='$pl[mineral]' WHERE nombre='$pl[nombre]'")or die(mysql_error());
	   		 echo "Los robots han extraido <b>$_GET[pot] minerales</b> con �xito.";
	   }
	}
   ...
   echo "<br><br><small>Cada W de potencia que gastes supondr� una Unidad de Mineral extraido</small>";
