...
if ($cli[nombre]!=""){	 
  if ($ok){
   ...
   if ($_GET[turnos]<=0){
	  ...
	  echo "<small>El clan $cli[nombre] te pagar� $cli[pago] por cada punto de Energ�a que est�s trabajando.<br>El clan ganar� un W por cada turno que trabajes para ellos</small>";
	  echo "<small><br><br>Recuerda que trabajar en el generador aumenta tus puntos hacia el lado de la Luz</small>";
	  echo '<form action="ciudad/" method="GET">Gastar <input name="turnos" type="input" value="1">Energ�a. <input name="id" type="hidden" value="igenerador"><input name="ok" value="Trabajar" type="submit"></form>';
  }

}else{
			echo "Mientras la Ciudad no tenga un clan, no se podr� trabajar en el generador"; 
}
?>	
