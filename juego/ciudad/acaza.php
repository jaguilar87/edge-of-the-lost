...
if ($us[hp]<=0){
	...
	if ($_POST[tr]) {
		...
		if ($us[turnos] < 0) {
			...
			echo "Tras estar un rato camuflado encuentras un $_POST[tr] y te abalanzas sobre �l!<br>";
		...
		if($luk>0){
			...
			mysql_query("INSERT INTO sw_inventario (jugador, objeto, equipable, tipo) VALUES ('$us[nombre]', 'Cad�ver $_POST[tr]', 'N', 'pieza')")or die(mysql_error());
		}else{
			echo "Pero el $_POST[tr] salvaje se te resisti� y escap�"; 
			$us[estres]+=10;
		}

		$dam=mt_rand($dife, $dife*100)-$us[constitucion];
		if ($dam<0){$dam=0;}
		echo "<br>El $_POST[tr] te da�� <b>$dam Puntos de Vida</b>";
