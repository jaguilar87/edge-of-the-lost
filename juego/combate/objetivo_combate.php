<?php
	...
	if ($us[nv_sable]==0){
		 echo "En tu rango aun no puedes atacar, debes ser Padawan/Aprendiz como m�nimo para poder atacar, pasate por la academia para subir de rango";
	}else{
		if (!$_GET[ob]){
?>
		 		Elige a un oponente digno de t� en la ciudad:<br>
				<table width="100%">
						<tr bgcolor="#4f4f4f">
							<td><b>Nombre</b></td>
							<td align="center"><b>M�rito</b></td>
							<td align="center"><b>Cr�ditos</b></td>
					...
					echo 'Ese jugador no existe';
						...
						echo 'Energ�a insuficiente....';
				  	 	...
				  	 	if ($ob[planeta]!=$us[planeta] || $ob[ciudad]!=$us[ciudad] || $us[nv_sable]!=$ob[nv_sable]){
					 		echo 'No puedes atacarle, no est� en tu ciudad o no es de tu nivel';
						}else{
							if ($ob[hp]<=0){
								echo "$ob[nombre] ya est� muerto, dejalo en paz";
							}else{
								if ($us[hp]<=0){
									echo "�Sabes que no eres inmortal verdad? No puedes atacar estando KO";
								}else{
