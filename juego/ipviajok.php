...
if ($ok){
   if ($_GET[ci]!="" && $_GET[pl]!=""){
			  				   ...
			  				   If ($vev[tprop]=="Jugador"){
			 						 ...
			 						 $viv=mysql_fetch_array($result);
 			 						 ...
 			 						 if($_GET[modo]!=$viv[vehiculo]){
									 		echo 'Ruta erronea... <script> location.href="ipviaje.php" </script>';
									 }else{
									 		 $coste=$viv[precio];
			 						 }
	  ...
	  if ($_GET[clan] && $vev[tprop]=="Jugador" && $us[nombre]==$vev[prop] && $us[nombre]==$cl[lider]){
	   ...
	   echo "<script> location.href='ipviaje.php' </script>";
}
	  ...
	  if ($_GET[ci]!="" && $_GET[pl]!=""){
		 ...
		 if ($us[nombre]==$cl[lider]){
		 	echo '<br><br><input type="checkbox" name="clan" value="Check me">Traerse a todo el Clan (Sólo con vehículo propio)';
		 }
