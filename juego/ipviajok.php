...
$c="SELECT * FROM sw_city WHERE nombre='$_GET[ci]' AND planeta='$plc[nombre]'";
...
default: 
			 ...
			 If ($vev[tprop]=="Jugador"){
 			 ...
 			 $c="SELECT * FROM sw_viaje WHERE vehiculo='$_GET[modo]' AND origen='$us[ciudad]' AND destino='$_GET[ci]'";
			 $result=mysql_query($c)or die(mysql_error());
			 $viv=mysql_fetch_array($result);
			 
 			 if($_GET[modo]!=$viv[vehiculo]){echo 'Ruta erronea... <script> location.href="ipviaje.php" </script>';}else{
			 $coste=$viv[precio];
			 }
...
if ($_GET[clan] && $vev[tprop]=="Jugador" && $us[nombre]==$vev[prop] && $us[nombre]==$cl[lider]){
   $c= "UPDATE sw_users SET ciudad='$_GET[ci]', planeta='$plc[nombre]' WHERE clan='$us[clan]' AND ciudad='$us[ciudad]'";
   $res=mysql_query($c);
}else{
	  $c= "UPDATE sw_users SET ciudad='$un[nombre]', planeta='$plc[nombre]' WHERE nombre='$us[nombre]'";
	  $res=mysql_query($c);

	  if ($vev[tprop]=="Clan"){
 			 $c="SELECT * FROM sw_clan WHERE nombre='$viv[clan]'";
			 $result=mysql_query($c)or die(mysql_error());
			 $clvi=mysql_fetch_array($result);
			 
			 $clvi[fondos]+=$costes;
			 
	  		 $c= "UPDATE sw_clan SET fondos='$clvi[fondos]'WHERE nombre='$clvi[nombre]'";
	  		 $res=mysql_query($c); 
	  }
...
}else{ echo "La ciudad no existe";}
}else{ echo "<script> location.href='ipviaje.php' </script>";}
...
echo '<form action="ipviajok.php" method="GET">
...
$c="SELECT * FROM `sw_viaje` WHERE origen='$us[ciudad]' AND destino='$_GET[ci]'";
$result=mysql_query($c)or die(mysql_error());
