...
$or=mysql_fetch_array($result);
	  ...
	  $c="SELECT * FROM sw_plan WHERE nombre='$or[planeta]'";
	  $result=mysql_query($c)or die(mysql_error());
...
$de=mysql_fetch_array($result);
	  ...
	  $c="SELECT * FROM sw_plan WHERE nombre='$de[planeta]'";
	  $result=mysql_query($c)or die(mysql_error());
...
echo 'Viaje borrado... <a href="cviajes.php">Volver</a>.';
}
...
echo '<form action="cviajes.php" METHOD="GET"><br><br>Ciudad Origen: <select name="ori">';
...
echo "<br><small><b>$v[vehiculo]</b> - de $v[origen] a $v[destino] - $v[precio] C</small> <a href=\"cviajes.php?ac=borrar&id=$v[id]\"><span title=\"borrar\"><img border=0 src=\"images/x.gif\"></a>";
}
