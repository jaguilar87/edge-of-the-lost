...
$c="UPDATE `sw_users` SET attmess='N' WHERE nombre='$_SESSION[nombre]'";
...
echo "Mostrando los últimos 20 Ataques:<br>";
	  ...
	  echo "<b>$a[id]-</b> <a href=\"ataque/lista.php?at=$a[id]\">$a[atacante] Vs $a[defensor] ocurrido el dia $a[dia]</a><br>";
      }

if ($us[clan]!=""){

	  echo "<a name=\"clan\"></a><br><hr><br>Mostrando los últimos 20 Ataques a ciudades de tu clan:<br>";
	  $c="SELECT * FROM `sw_att` WHERE atacante='$cl[nombre]' OR defensor='$cl[nombre]' ORDER BY id DESC limit 0,20"; 
	  $result=mysql_query($c)or die(mysql_error());
	  while ($a = mysql_fetch_array($result)){
	  echo "<b>$a[id]-</b> <a href=\"ataque/lista.php?at=$a[id]\">Ataque contra $a[defensor] ocurrido el dia $a[dia]</a><br>";
      }
}	  
