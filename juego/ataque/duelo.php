<?php include_once 'header.php';
	  ...
	  if ($us[nivel]=="Usuario de la Fuerza"){echo 'A tu nivel aun no puedes hacer duelos...';}else{
	 	if ($us[clan]==""){echo "Debes pertenecer a un clan para retar a duelos";}else{

	  	 if ($_GET[ob]==""){echo 'Para hacer un duelo debes elegir alguien de tu clan o clanes aliados<br>';

		 	   echo '<table width="100%" cellpading="5" cellspacing="5" ><tr bgcolor="#4f4f4f"><td><b>'.$us[clan].'</b></td></tr>';
			
		       $c="SELECT * FROM `sw_users` WHERE clan='$us[clan]' ORDER BY puntos DESC";
			   $result=mysql_query($c)or die(mysql_error());
			   $i=1;
			   while ($r = mysql_fetch_array($result)){$u=textcolor($r[nombre]); echo "<tr><td><a href=\"ataque/duelo.php?ob=$r[nombre]\"><img border=0 src=\"images/atk.gif\"></a><b>$i.</b> <a href=\"lista/info.php?us=$r[nombre]\"> <font color=\"$u[txtc]\">$r[titulo] $r[nombre]</font></a></td></tr>"; $i++;}
			...
			if ($ob[nombre]==$us[nombre]){Echo 'No puedes atacarte a ti mismo';}else{
			   if ($ob[nombre]==""){echo 'Ese jugador no existe';}else{			   
					...
					if ($r[estado]=="Aliado" || $ob[clan]==$us[clan]){
			   		   ...
			   		   include 'ataque/atk.php';
			   		   include 'ataque/duelo_res.php';
					}else{
