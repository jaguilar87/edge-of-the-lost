<?php include_once 'header.php';
	  if ($us[nivel]=="Usuario de la Fuerza"){echo 'A tu nivel aun no puedes atacar...';}else{
	  	 if ($_GET[ob]==""){echo 'Para atacar debes elegir un oponente de tu ciudad:<br>';
			...
			$c="SELECT * FROM `sw_users` WHERE planeta='$us[planeta]' AND reg='S' AND hp>'0'  AND ciudad='$ci[nombre]' AND nv_sable='$us[nv_sable]' ORDER BY puntos DESC";
			$result=mysql_query($c)or die(mysql_error());
			$i=1;
			while ($r = mysql_fetch_array($result)){$u=textcolor($r[nombre]); echo "<tr><td><a href=\"ataque/combate.php?ob=$r[nombre]\"><img border=0 src=\"images/atk.gif\"></a><b>$i.</b> <a href=\"lista/info.php?us=$r[nombre]\"> <font color=\"$u[txtc]\"><small>$r[titulo] $r[nombre]</small></font></a></td><td><b><center><small>$r[puntos]</small></center></b></td><td><div align=\"right\"><small>$r[creditos]</small></div></td><td>$r[clan]</td></tr>"; $i++;}
			echo '</table>';
							 	...
							 	if ($us[hp]<=0){echo "¿Sabes que no eres inmortal verdad? No puedes atacar estando KO";}else{
								   if ($ob[clan]==$us[clan] && $us[clan]!=NULL){echo "No puedes atacar a personas de tu clan";}else{
										 ...
										 if ($r[estado]=="Aliado"){echo "El objetivo pertenece a un clan aliado, no lo puedes atacar...";}else{
											...
											include 'ataque/combate_res.php';		
										 }
									}
								}
							 }
