...
if ($us[nombre]==$cl[lider]) {
    if ($_GET[ve]!="" && $_GET[ori]!="" && $_GET[des]!="" && $_GET[pre]!="") {
        ...
        if ($ve[tipo]=="VCA" && $pde[nombre]!=$por[nombre]) {
            echo 'Ese tipo de veh&iacute;culo no es V&aacute;lido para ese viaje.';
        } else {
        ...
        echo '</select><br><br>Veh&iacute;culo: <select name="ve">';
        ...
        echo '</select><br><br>Precio: <input name="pre" type="text" value="200"> Cr&eacute;ditos<br><br><input type="submit" value="Crear Ruta"></form>';

        echo "<br><br><big><big><center>Gesti&oacute;n de Rutas ya existentes</center></big></big>";

        $c="SELECT * FROM sw_viaje WHERE clan='$us[clan]' ORDER BY vehiculo ASC";
        $result=mysql_query($c)or die(mysql_error());
        while ($v=mysql_fetch_array($result)) {
            echo "<br><small><b>$v[vehiculo]</b> - de $v[origen] a $v[destino] - $v[precio] C</small> <a href=\"clan/?id=viajes&ac=borrar&did=$v[id]\"><span title=\"borrar\"><img border=0 src=\"images/x.gif\"></a>";
        }
    }
} else {
